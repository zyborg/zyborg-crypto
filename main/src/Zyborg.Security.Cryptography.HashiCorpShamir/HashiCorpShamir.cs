using System;
using System.Collections.Generic;
using System.Security.Cryptography;

namespace Zyborg.Security.Cryptography
{
    public static class HashiCorpShamir
    {
        // Split takes an arbitrarily long secret and generates a `parts`
        // number of shares, `threshold` of which are required to reconstruct
        // the secret. The parts and threshold must be at least 2, and less
        // than 256. The returned shares are each one byte longer than the secret
        // as they attach a tag used to reconstruct the secret.
        public static byte[][] Split(byte[] secret, int parts, int threshold)
        {
            // Sanity check the input
            if (parts < threshold)
                throw new ArgumentOutOfRangeException(nameof(parts),
                        "parts cannot be less than threshold");

            if (parts > 255)
                throw new ArgumentOutOfRangeException(nameof(parts),
                        "parts cannot exceed 255");

            if (threshold < 2)
                throw new ArgumentOutOfRangeException(nameof(threshold),
                        "threshold must be at least 2");
            if (threshold > 255)
                throw new ArgumentOutOfRangeException(nameof(threshold),
                        "threshold cannot exceed 255");

            if ((secret?.Length).GetValueOrDefault() == 0)
                throw new ArgumentException(nameof(secret),
                        "cannot split an empty secret");


            // Allocate the output array, initialize the final byte
            // of the output with the offset. The representation of each
            // output is {y1, y2, .., yN, x}.
            var ret = new byte[parts][];
            for (int idx = 0; idx < ret.Length; idx++)
            {
                ret[idx] = new byte[secret.Length + 1];
                ret[idx][secret.Length] = (byte)(idx + 1);
            }

            // Construct a random polynomial for each byte of the secret.
            // Because we are using a field of size 256, we can only represent
            // a single byte as the intercept of the polynomial, so we must
            // use a new polynomial for each byte.
            for (int idx = 0; idx < secret.Length; idx++)
            {
                var val = secret[idx];
                var p = MakePolynomial(val, (byte)(threshold - 1));

                // Generate a `parts` number of (x,y) pairs
                // We cheat by encoding the x value once as the final index,
                // so that it only needs to be stored once.
                for (int i = 0; i < parts; i++)
                {
                    var x = (byte)(i + 1);
                    var y = p.Evaluate(x);
                    ret[i][idx] = y;
                }
            }

            // Return the encoded secrets
            return ret;
        }

        // Combine is used to reverse a Split and reconstruct a secret
        // once a `threshold` number of parts are available.
        public static byte[] Combine(byte[][] parts)
        {
            // Verify enough parts provided
            if ((parts?.Length).GetValueOrDefault() < 2)
                throw new ArgumentOutOfRangeException(nameof(parts),
                        "less than two parts cannot be used to reconstruct the secret");

            // Verify the parts are all the same length
            var firstPartLen = parts[0].Length;
            if (firstPartLen < 2)
                throw new ArgumentException(nameof(parts),
                        "parts must be at least two bytes");

            for (int i = 1; i < parts.Length; i++)
            {
                if (parts[i].Length != firstPartLen)
                    throw new ArgumentException(nameof(parts),
                            "all parts must be the same length");
            }

            // Create a buffer to store the reconstructed secret
            var secret = new byte[firstPartLen - 1];

            // Buffer to store the samples
            var x_samples = new byte[parts.Length];
            var y_samples = new byte[parts.Length];

            // Set the x value for each sample and ensure no x_sample values are the same,
            // otherwise div() can be unhappy
            var checkMap = new Dictionary<byte, bool>();
            for (int i = 0; i < parts.Length; i++)
            {
                var part = parts[i];
                var samp = part[firstPartLen - 1];
                if (checkMap.ContainsKey(samp))
                    throw new InvalidOperationException("duplicate part detected");

                checkMap[samp] = true;
                x_samples[i] = samp;
            }

            // Reconstruct each byte
            for (int idx = 0; idx < secret.Length; idx++)
            {
                // Set the y value for each sample
                for (int i = 0; i < parts.Length; i++)
                {
                    var part = parts[i];
                    y_samples[i] = part[idx];
                }

                // Interpolte the polynomial and compute the value at 0
                var val = HashiCorpShamir.InterpolatePolynomial(x_samples, y_samples, 0);

                // Evaluate the 0th value to get the intercept
                secret[idx] = val;
            }
            return secret;
        }


        // makePolynomial constructs a random polynomial of the given
        // degree but with the provided intercept value.
        public static Polynomial MakePolynomial(byte intercept, byte degree)
        {
            // Create a wrapper
            var p = new Polynomial
            {
                Coefficients = new byte[degree + 1],
            };

            // IMPL NOTE:  We swapped the order of the following two ops from the original HC
            // impl because C# doesn't have a "slice ref" ability to make the operations simpler
            // p.coefficients[0] = intercept
            // if _, err := rand.Read(p.coefficients[1:]); err != nil {
            //     return p, err
            // }            

            // Assign random co-efficients to the polynomial
            using (var rng = RandomNumberGenerator.Create())
            {
                rng.GetBytes(p.Coefficients);
            }

            // Ensure the intercept is set
            p.Coefficients[0] = intercept;

            return p;
        }

        // evaluate returns the value of the polynomial for the given x
        public static byte Evaluate(this Polynomial p, byte x)
        {
            // Special case the origin
            if (x == 0)
            {
                return p.Coefficients[0];
            }

            // Compute the polynomial value using Horner's method.
            var degree = p.Coefficients.Length - 1;
            var ret = p.Coefficients[degree];
            for (var i = degree - 1; i >= 0; i--)
            {
                var coeff = p.Coefficients[i];
                ret = Add(Mult(ret, x), coeff);
            }

            return ret;
        }

        // interpolatePolynomial takes N sample points and returns
        // the value at a given x using a lagrange interpolation.
        public static byte InterpolatePolynomial(byte[] x_samples, byte[] y_samples, byte x)
        {
            var limit = x_samples.Length;
            byte result = 0, basis;

            for (int i = 0; i < limit; i++)
            {
                basis = 1;
                for (int j = 0; j < limit; j++)
                {
                    if (i == j)
                    {
                        continue;
                    }

                    var num = Add(x, x_samples[j]);
                    var denom = Add(x_samples[i], x_samples[j]);
                    var term = Div(num, denom);
                    basis = Mult(basis, term);
                }
                var group = Mult(y_samples[i], basis);
                result = Add(result, group);
            }
            return result;
        }

        // div divides two numbers in GF(2^8)
        public static byte Div(byte a, byte b)
        {
            if (b == 0)
            {
                // leaks some timing information but we don't care anyways as this
                // should never happen, hence the panic
                throw new DivideByZeroException();
            }

            byte goodVal, zero = 0;
            var log_a = LOG_TABLE[a];
            var log_b = LOG_TABLE[b];
            var diff = (((int)log_a) - ((int)log_b)) % 255;
            if (diff < 0)
            {
                diff += 255;
            }

            var ret = EXP_TABLE[diff];

            // Ensure we return zero if a is zero but aren't subject to timing attacks
            goodVal = ret;

            // if subtle.ConstantTimeByteEq(a, 0) == 1 {
            if (a == 0)
            {
                ret = zero;
            }
            else
            {
                ret = goodVal;
            }

            return ret;
        }

        // mult multiplies two numbers in GF(2^8)
        public static byte /* (out uint8)*/ Mult(byte a, byte b)
        {
            byte goodVal, zero = 0;

            var log_a = LOG_TABLE[a];
            var log_b = LOG_TABLE[b];
            var sum = (((int)log_a) + ((int)log_b)) % 255;

            var ret = EXP_TABLE[sum];

            // Ensure we return zero if either a or be are zero but aren't subject to
            // timing attacks
            goodVal = ret;

            // if (subtle.ConstantTimeByteEq(a, 0) == 1)
            if (a == 0)
            {
                ret = zero;
            }
            else
            {
                ret = goodVal;
            }

            // if subtle.ConstantTimeByteEq(b, 0) == 1 {
            if (b == 0)
            {
                ret = zero;
            }
            else
            {
                // This operation does not do anything logically useful. It
                // only ensures a constant number of assignments to thwart
                // timing attacks.
                goodVal = zero;
            }

            return ret;
        }

        // add combines two numbers in GF(2^8)
        // This can also be used for subtraction since it is symmetric.
        public static byte Add(byte a, byte b)
        {
            return (byte)(a ^ b);
        }

        // polynomial represents a polynomial of arbitrary degree
        public struct Polynomial
        {
            public byte[] Coefficients;
        }

        // Tables taken from http://www.samiam.org/galois.html
        // They use 0xe5 (229) as the generator

    	// logTable provides the log(X)/log(g) at each index X
        public static byte[] LOG_TABLE =
        {
            
        };

        // expTable provides the anti-log or exponentiation value
        // for the equivalent index
        public static byte[] EXP_TABLE = 
        {
            
        };
    }
}